...
the others. Bob observed that one number usually differs from the others 
...
function iqTest(numbers) {
  ...
  const numArr = numbers.split(" "); // transform into array

  // put index of even and odd numbers into separate arrays
  ...
  // determine which one is shorter (i.e. only has 1 item)
  if (evenNums.length < oddNums.length) {
    uniqueIndex = evenNums[0];
  } else {
    uniqueIndex = oddNums[0];
  }
